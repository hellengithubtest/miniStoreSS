...
<ul class="pagination">
    ...
    <c:otherwise>
        <li class="page-item">
              <a class="page-link" href="${pageContext.request.contextPath}/products?page=${number - 1}&filter=${filter}&size=${size}">Previous</a>
        </li>
    </c:otherwise>
    </c:choose>
        ...
        <c:choose>
         <c:when test="${number == tagStatus.index - 1}">
            <li class="page-item active">
                  <a class="page-link" href="${pageContext.request.contextPath}/products?page=${tagStatus.index - 1}&filter=${filter}&size=${size}">${tagStatus.index}<span class="sr-only">(current)</span></a>
            </li>
         </c:when>
         <c:otherwise>
            ...
            <c:choose>
                <c:when test="${tagStatus.index == current}">
                           <li class="page-item"><a class="page-link" href='${pageContext.request.contextPath}/products?page=${tagStatus.index - 1}&filter=${filter}&size=${size}'>${tagStatus.index}</a>
                           </li>
                </c:when>
...
<c:choose>
    ...
    <c:otherwise>
        <li class="page-item">
             <a class="page-link" href="${pageContext.request.contextPath}/products?page=${number + 1}&filter=${filter}&size=${size}">Next</a>
        </li>
    </c:otherwise>
</c:choose>
</ul>
